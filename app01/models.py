...
class UserInfo(AbstractUser):
    ...
    user = models.OneToOneField(to=User1,null=True,blank= True,on_delete=models.CASCADE)
...
class Customer(models.Model):
    ...
    # null= True 是限制数据库里的   blank = True 是限制 ModelForm 的
    class_list = models.ManyToManyField('ClassList', verbose_name="已报班级",blank=True )
    baoming_date = models.DateField("报名时间", null = True,blank=True)

    def __str__(self):
        return self.name+str(self.baoming_date)

    # 以后有 一个东西如果对应多个值就可以这么做非常nice
...
class ConsultRecord(models.Model):
    ...
    # limit_choices_to 与数据库的没有关系, MOdelForm渲染的时候才有用
